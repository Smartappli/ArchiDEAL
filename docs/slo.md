...
to reach metrics ports 44180, 9091, 8080, 7000-7002 and 9100. Scraping from another namespace will
fail closed.

The monitors assign stable `job` labels for APISIX, oauth2-proxy, the bridge, DEALData APIs and
DEALData consumers. Do not build alerts from generated ServiceMonitor job names. Alertmanager must
...
for complete bridge/consumer/edge target loss, commit/database failure, event-clock failure, DLQ or
rejection ratio, Kafka acknowledgement latency, freshness and APISIX/etcd reachability are included.
