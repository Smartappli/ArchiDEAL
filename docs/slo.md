...
| Runtime management health | Controller `up` and Kubernetes-ready gauge; worker loop liveness, heartbeat/database readiness; durable queue depth, oldest age and stale-lease gauges | Operational coverage for controller/worker loss and stalled or growing operation queues. The worker metrics Service retains NotReady endpoints for diagnosis. These gauges measure the control plane, not application request availability. |
